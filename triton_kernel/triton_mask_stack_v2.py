...
def _create_mask_stack(
    ...
    start_k = k_id * BLOCK_K
    end_k = min((k_id + 1) * BLOCK_K, KERNEL_NUM)
    for cur_k_id in range(start_k, end_k):
        for i in range((KV_LEN + BLOCK_N - 1) // BLOCK_N):
            ...
            indices_val = indices_val == cur_k_id # [BLOCK_N]
            mask_stack_store_offset = cur_k_id * mask_stack_stride0 + b_id * mask_stack_stride1 + h_id * mask_stack_stride2 + i * BLOCK_N + tl.arange(0, BLOCK_N) # [BLOCK_N]
            mask_stack_store_offset_max = cur_k_id * mask_stack_stride0 + b_id * mask_stack_stride1 + h_id * mask_stack_stride2 + KV_LEN # [1]
...
def triton_create_mask_stack(
    ...
    grid = lambda meta: ((KERNEL_NUM + BLOCK_K - 1) // BLOCK_K, BSZ, NUM_HEAD)

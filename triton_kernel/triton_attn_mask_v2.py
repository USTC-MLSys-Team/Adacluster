...
def _create_sparse_attn_mask(
    ...
    tmp_mask = tl.zeros((BLOCK_M, BLOCK_N), dtype=tl.int8)
    for i in range(TOPK_NUM):
        ...
        mask_stack_offset = (topk_indices_val * mask_stack_stride0)[:, None] + bid * mask_stack_stride1 + head_id * mask_stack_stride2 + kv_id * BLOCK_N + tl.arange(0, BLOCK_N) # [BLOCK_M, BLOCK_N]
        loaded_mask = tl.load(mask_stack_ptr + mask_stack_offset) # [BLOCK_M, BLOCK_N]

        tmp_mask = tmp_mask | loaded_mask
    
    cur_attention_mask_offset = attention_mask_offset + (q_id * BLOCK_M + tl.arange(0, BLOCK_M)) * attention_mask_stride2 # [BLOCK_M]
    cur_attention_mask_offset = cur_attention_mask_offset[:, None] + kv_id * BLOCK_N + tl.arange(0, BLOCK_N) # [BLOCK_M, BLOCK_N]
    
    cur_attention_mask_offset_q_max = attention_mask_offset + Q_LEN * attention_mask_stride2 # [1]
    cur_attention_mask_offset_q_max = cur_attention_mask_offset_q_max[:, None] + kv_id * BLOCK_N + tl.arange(0, BLOCK_N) # [1, BLOCK_N]
    ...
    tl.store(attention_mask_ptr + cur_attention_mask_offset, tmp_mask, mask=(mask_1 & mask_2)) # [BLOCK_M, BLOCK_N]
...
#         triton_attention_masks = torch.zeros(BSZ, NUM_HEAD, Q_LEN, KV_LEN, device=query_states.device, dtype=torch.int8)
...
#     torch.cuda.synchronize()
#     ed_time = time.time()
#     print(f"triton create time: {(ed_time - st_time) / bench_time * 1000:.2f} ms")
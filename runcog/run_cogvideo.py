...
# from cv_utils.cogvideo_attn_processor.cog_video_attn_processor_kvclus import CogVideoXAttnProcessor2_0_kvclus
from modify_cogvideox.cog_video_attn_processor_kvclus_new_old import CogVideoXAttnProcessor2_0_kvclus
from modify_cogvideox.cog_video_attn_processor_full_compare import CogVideoXAttnProcessor2_0_compare
from modify_cogvideox.cog_video_attn_processor_svg2 import CogVideoXAttnProcessor2_0_SVG2
from modify_cogvideox.pipeline_cogvideox import CogVideoXPipelineSparse

logging.basicConfig(level=logging.INFO)

from modify_cogvideo import set_spas_sage_attn_cogvideox
...
def generate_video(
    ...
    if generate_type == "i2v":
        ...
        if method == "kvclus":
            sparse_attn_processor=CogVideoXAttnProcessor2_0_kvclus()
            pipe = CogVideoXPipelineSparse.from_pretrained(model_path, torch_dtype=dtype)
            pipe.set_sparse_attn_processor(sparse_attn_processor)
            pipe.set_skip_ratio(skip_ratio)
        elif method == "svg2":
            sparse_attn_processor=CogVideoXAttnProcessor2_0_SVG2()
            pipe = CogVideoXPipelineSparse.from_pretrained(model_path, torch_dtype=dtype)
            pipe.set_sparse_attn_processor(sparse_attn_processor)
            pipe.set_skip_ratio(skip_ratio)
        elif method == "full":
    ...
    if use_spas_sage_attn:
        set_spas_sage_attn_cogvideox(pipe.transformer, verbose=False, l1=0.05, pv_l1=0.06)
        # load saved state_dict
        saved_state_dict = torch.load("/workspace/SpargeAttn/CogVideoX-2b_0.06_0.07.pt", weights_only=True)  
        load_sparse_attention_state_dict(pipe.transformer, saved_state_dict)
    ...
    if generate_type == "i2v":
        ...
        # torch.cuda.memory._dump_snapshot("mem_usage.pickle")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fps", type=int, default=8, help="The frames per second for the generated video"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--method", type=str, default="kvclus", choices=["full", "kvclus", "svg2"], help="The sparse algorithm"
    )
    parser.add_argument(
        "--skip_ratio", type=float, default=0.3, help="skip ratio in denoising step"
    )
    ...
    parser.add_argument(
        "--use_spas_sage_attn",
        action="store_true",
        help="Use Sparge attention for the model",
    )
    ...
    generate_video(
        ...
        seed=args.seed,
        fps=args.fps,
        use_spas_sage_attn=args.use_spas_sage_attn,
        method=args.method,

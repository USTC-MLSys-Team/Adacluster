...
from .flashinfer_sparse_attn import flashinfer_cluster_sparse_attn
...
class WanSelfAttention(nn.Module):
    def __init__(self,
        ...
        self.mse_values = []
        # 动态确定的聚类数量
        self.q_kernel_num = None
        self.kv_kernel_num = None
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, topk_num: int = 128, q_kernel_num: int = 100, kv_kernel_num: int = 500):
        ...
        # 3. 动态确定聚类数量（第一次调用时）或使用已确定的数量
        if self.prev_centroid is None:
            print(f"Layer {self.layer_idx} first call, determining cluster numbers...")
            print(f"q shape: {q.shape}, k shape: {k.shape}")
            
            # 第一次调用：使用 thresholded_kmeans_loop 确定聚类数量
            initial_kkernel_num = 200  # 初始聚类中心数量（K）
            initial_qkernel_num = 50   # 初始聚类中心数量（Q）
            
            # 确定 KV 的聚类数量
            random_indices_for_kv_initial = torch.randperm(k.shape[2], device=x.device)[:initial_kkernel_num]
            key_initial_kernel = k[:, :, random_indices_for_kv_initial, :].clone()
            
            kv_kernel_num_determined = thresholded_kmeans_loop(
            ...
            if kv_kernel_num_determined == -1:
                # 使用全注意力
                self.use_full_attention = True
                print(f"Layer {self.layer_idx} will use full attention (KV clustering failed)")
            else:
                self.kv_kernel_num = kv_kernel_num_determined
                print(f"Layer {self.layer_idx} determined KV kernel num: {self.kv_kernel_num}")
            
            # 确定 Q 的聚类数量
            if not self.use_full_attention:
                random_indices_for_query_initial = torch.randperm(q.shape[2], device=x.device)[:initial_qkernel_num]
                query_initial_kernel = q[:, :, random_indices_for_query_initial, :].clone()
                
                q_kernel_num_determined = thresholded_kmeans_loop(
                ...
                if q_kernel_num_determined == -1:
                    # 使用全注意力
                    self.use_full_attention = True
                    print(f"Layer {self.layer_idx} will use full attention (Q clustering failed)")
                else:
                    self.q_kernel_num = q_kernel_num_determined
                    print(f"Layer {self.layer_idx} determined Q kernel num: {self.q_kernel_num}")
            ...
            # 使用确定的聚类数量初始化聚类中心
            random_indices_for_kv = torch.randperm(k.shape[2], device=x.device)[:self.kv_kernel_num]
            random_indices_for_q = torch.randperm(q.shape[2], device=x.device)[:self.q_kernel_num]
            ...
            print(f"Layer {self.layer_idx} initialization complete: Q kernel num = {self.q_kernel_num}, KV kernel num = {self.kv_kernel_num}")
        else:
            # 后续调用：使用已确定的聚类数量
            if self.use_full_attention:
                # 使用全注意力
        ...
        hidden_states = flashinfer_cluster_sparse_attn(

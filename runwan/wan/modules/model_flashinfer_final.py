...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, topk_num: int = 128, q_kernel_num: int = 100, kv_kernel_num: int = 500):
        ...
        # 4. 使用 FlashInfer 计算 attention
        sys.path.insert(0, "/workspace")
        import flashinfer
        
        batch_size, num_heads, seq_len, head_dim = q.shape
        
        # FlashInfer 需要 float16/bfloat16，不支持 float32
        dtype = torch.float16 if q.dtype == torch.float32 else q.dtype
        q_flashinfer = q.flatten(0, 1).permute(1, 0, 2).contiguous().to(dtype)
        k_flashinfer = k.flatten(0, 1).permute(1, 0, 2).contiguous().to(dtype)
        v_flashinfer = v.flatten(0, 1).permute(1, 0, 2).contiguous().to(dtype)
        
        output, _ = flashinfer.single_prefill_with_kv_cache(
            q_flashinfer, k_flashinfer, v_flashinfer,
            causal=False, return_lse=True,
        )
        
        hidden_states = output.permute(1, 0, 2).reshape(batch_size, num_heads, seq_len, head_dim)
        
        # 转回原始数据类型
        hidden_states = hidden_states.to(q.dtype)
        
        # 5. 输出投影: [B, H, S, D] -> [B, S, H*D]
        hidden_states = hidden_states.permute(0, 2, 1, 3).contiguous().view(b, s, n * d)
        output = self.o(hidden_states)
